...
?>
<?php
function add_premium_user_meta() {
    if ( is_user_logged_in() ) {
           ...
           $pass_expiration = get_option('passexpiration'); 

           if ( delete_user_meta($user_id, '_paid_premium') ) {
             add_user_meta( $user_id, '_paid_premium', $pass_expiration);             
  
         } else {
             add_user_meta( $user_id, '_paid_premium', $pass_expiration);
            
         }
        $user_status = get_user_meta( $user_id, '_paid_premium', $single );        
         
    }
}
add_action( 'add_premium_user', 'add_premium_user_meta' );

?>
<?php
function add_basic_user_meta() {
    if ( is_user_logged_in() ) {
    
           global $current_user, $pass_expiration;
           $current_user = wp_get_current_user();  
           $single = true; 
           $user_id =  $current_user->ID;
           $pass_expiration = get_option('passexpiration'); 

           if ( delete_user_meta($user_id, '_paid_basic') ) {
             add_user_meta( $user_id, '_paid_basic', $pass_expiration);             
  
         } else {
             add_user_meta( $user_id, '_paid_basic', $pass_expiration);
            
         }
        $user_status = get_user_meta( $user_id, '_paid_basic', $single );
                       
    }
}
add_action( 'add_basic_user', 'add_basic_user_meta' );

?>
<?php
function add_free_user_meta() {
    if ( is_user_logged_in() ) {
           ...
           if ( delete_user_meta($user_id, '_paid_free') ) {
             add_user_meta( $user_id, '_paid_free', $pass_expiration);             
  
         } else {
             add_user_meta( $user_id, '_paid_free', $pass_expiration);
            
         }
        $user_status = get_user_meta( $user_id, '_paid_free', $single );
                       
    }
}
add_action( 'add_free_user', 'add_free_user_meta' );
?>
<?php
function add_light_user_meta() {
    if ( is_user_logged_in() ) {
           ...
           if ( delete_user_meta($user_id, '_paid_light') ) {
             add_user_meta( $user_id, '_paid_light', $pass_expiration);             
  
         } else {
             add_user_meta( $user_id, '_paid_light', $pass_expiration);
            
         }
        $user_status = get_user_meta( $user_id, '_paid_light', $single );
                       
    }
}
add_action( 'add_light_user', 'add_light_user_meta' );
...
function add_gcf_interface() {
	add_options_page('600Gestures 3D', '600Gestures 3D', '8', 'functions', 'editglobalcustomfields');
}

function editglobalcustomfields() {
	?>
	<div class='wrap'>
	<h2>600Gestures 3D Settings</h2>
	<form method="post" action="options.php">
	<?php wp_nonce_field('update-options') ?>

	<p><strong>600Gestures 3D Pass Expiration:</strong><br />
	<input type="text" name="passexpiration" size="45" value="<?php echo get_option('passexpiration'); ?>" /></p>

	<p><input type="submit" name="Submit" value="Update" /></p>

	<input type="hidden" name="action" value="update" />
	<input type="hidden" name="page_options" value="passexpiration" />
